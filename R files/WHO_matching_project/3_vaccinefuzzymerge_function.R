...
#write.csv(df_who, "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/Data_request_20171201_geomatch.csv")

#---------------------Clean up data set--------------------------------#
#load shapefile files (Gadm and gaul) Admin 2/ old gadm shapefile
df_gadm <- fread(paste0(input_dir, "lf_gadm28_adm2.csv"))
df_gl <- fread(paste0(input_dir, "lf_g2015_2014_2_addiso3.csv"))

#load new gadm36 admin 2 shapefile
df_gadm36 <- fread(paste0(input_dir, "gadm_36_ad2.csv"))


#For shapefile, select column of interest and and select country of interest
df_gadm <- subset(df_gadm, select = c(3,4,6,8,19)) 
df_gl <- subset(df_gl, select = c(2,6,10,11,15))
df_gadm36 <- subset(df_gadm36, select = c(9,11,13,14))
setnames(df_gadm, c("NAME_0","NAME_1","NAME_2"), c("ADM0_NAME","ADM1_NAME", "ADM2_NAME"))
...
# Clean up names
#fix diacritics
df_who[, ADM0_NAME := fix_diacritics(ADM0_NAME)]
df_gadm[, ADM0_NAME := fix_diacritics(ADM0_NAME)]
df_gl[, ADM0_NAME := fix_diacritics(ADM0_NAME)]
df_gadm36[, ADM0_NAME := fix_diacritics(ADM0_NAME)]

df_who[, ADM1_NAME := fix_diacritics(ADM1_NAME)]
df_gadm[, ADM1_NAME := fix_diacritics(ADM1_NAME)]
df_gl[, ADM1_NAME := fix_diacritics(ADM1_NAME)]
df_gadm36[, ADM1_NAME := fix_diacritics(ADM1_NAME)]

df_who[, ADM2_NAME := fix_diacritics(ADM2_NAME)]
df_gadm[, ADM2_NAME := fix_diacritics(ADM2_NAME)]
df_gl[, ADM2_NAME := fix_diacritics(ADM2_NAME)]
df_gadm36[, ADM1_NAME := fix_diacritics(ADM2_NAME)]

#change to lower case
df_who[, ADM1_NAME := tolower(ADM1_NAME)]
df_gadm[, ADM1_NAME := tolower(ADM1_NAME)]
df_gl[, ADM1_NAME := tolower(ADM1_NAME)]
df_gadm36[, ADM1_NAME := tolower(ADM1_NAME)]

df_who[, ADM2_NAME := tolower(ADM2_NAME)]
df_gadm[, ADM2_NAME := tolower(ADM2_NAME)]
df_gl[, ADM2_NAME := tolower(ADM2_NAME)]
df_gadm36[, ADM2_NAME := tolower(ADM2_NAME)]

#change some symbol
df_who[, ADM2_NAME := gsub("[[:punct:]]", "", ADM2_NAME)]
df_gadm[, ADM2_NAME := gsub("[[:punct:]]", "", ADM2_NAME)]
df_gl[, ADM2_NAME := gsub("[[:punct:]]", "", ADM2_NAME)]
df_gadm36[, ADM2_NAME := gsub("[[:punct:]]", "", ADM2_NAME)]

df_who[, ADM1_NAME := gsub("[[:punct:]]", "", ADM1_NAME)]
df_gadm[, ADM1_NAME := gsub("[[:punct:]]", "", ADM1_NAME)]
df_gl[, ADM1_NAME := gsub("[[:punct:]]", "", ADM1_NAME)]
df_gadm36[, ADM1_NAME := gsub("[[:punct:]]", "", ADM1_NAME)]

df_who[, ADM2_NAME := gsub("[[:space:]]", "", ADM2_NAME)]
df_gadm[, ADM2_NAME := gsub("[[:space:]]", "", ADM2_NAME)]
df_gl[, ADM2_NAME := gsub("[[:space:]]", "", ADM2_NAME)]
df_gadm36[, ADM2_NAME := gsub("[[:space:]]", "", ADM2_NAME)]

df_who[, ADM1_NAME := gsub("[[:space:]]", "", ADM1_NAME)]
df_gadm[, ADM1_NAME := gsub("[[:space:]]", "", ADM1_NAME)]
df_gl[, ADM1_NAME := gsub("[[:space:]]", "", ADM1_NAME)]
df_gadm36[, ADM1_NAME := gsub("[[:space:]]", "", ADM1_NAME)]

#--------------------------------------Merging name adm 2 code---------------------------------------------------#
##Merge Africa countries with gadm shapefiles 

#count country
africa_countries <- unique(df_who$ADM0_NAME, by= "ADM0_NAME")
africa_iso3 <- unique(df_who$iso3, by = "iso3")
countries <- unique(df_gadm, by = "ADM0_NAME")
#match gadm/gaul shapefile with WHO data set 



# start the for loop matching for all african countries using gadm shapefile

matching_admin2 <- function(countries, shapefile, fuzMethod){
  df_combined <- list()
  df_who_only <- list()
...
for (i in 1 : length(countries)){
  df_combined[[i]]<- merge(df_who[ADM0_NAME == countries[i]], shapefile[ADM0_NAME == countries[i]], by = c("ADM1_NAME", "ADM2_NAME"), all.x = T, all.y = T)
  ...
  #fuzzy match
  df_fuzzy[[i]] <- stringdist_join(subset(df_who_only[[i]], select = c(1:6)), subset(df_shpfile_only[[i]], select = c(1,2,8,9,10,11)), by = "ADM2_NAME", method = fuzMethod)
  ...
  for (j in 1:(nrow(df_fuzzy[[i]])-1)){
    ...
    if  (nrow(df_fuzzy[[i]]) <= 1){
      next
    } else if (df_fuzzy[[i]]$ADM2_NAME.x[j] == df_fuzzy[[i]]$ADM2_NAME.x[j+1]) {
  ...
  setnames(df_fuzzy_fix[[i]], c("ADM2_NAME.x", "ADM2_NAME.y"), c("ADM2_NAME.who", "ADM2_NAME.shpfile"))
  # fuzzy match with only one shapefile location will have NA on merge status
  df_fuzzy_exact[[i]] <- df_fuzzy[[i]][is.na(merge_status)]
  setnames(df_fuzzy_exact[[i]], c("ADM2_NAME.x", "ADM2_NAME.y"), c("ADM2_NAME.who", "ADM2_NAME.shpfile"))
  ...
  df_combined[[i]][is.na(ADM2_NAME.who), ADM2_NAME.who := ADM2_NAME]
  # for location in shapefile but do not get match in who location
  df_combined[[i]] <- rbind(df_combined[[i]], df_shpfile_only[[i]], fill = T)
  df_combined[[i]][merge_status == "in shpfile but not WHO", ADM2_NAME.shpfile := ADM2_NAME]
  #sort out location in WHO which never get match (regular and fuzzy match)
  df_who_miss[[i]] <- unique(df_combined[[i]], by= c("who_coverage", "ADM2_NAME.who"))
  #sort out location in shapefile which not get mactch with WHO location
  df_shp_miss[[i]] <- unique(df_combined[[i]], by = c("ADM2_NAME.shpfile", "GAUL_CODE"))
...
gadm2_match <- matching_admin2(adm2_gadm_countries, df_gadm, "osa")
gaul2_match <- matching_admin2(adm2_gaul_countries, df_gl, "osa")


matching1 <- matching_admin2(country1, df_gadm2, "osa") %>% rbindlist(.)

setnames(df_who, "NAME", "ADM2_NAME")
NGA_matching <- matching_admin2("Nigeria", df_gadm2, "osa") %>% rbindlist(.)
write.csv(NGA_matching, paste0(input_dir2017,"NGA_match.csv"))

write.csv(matching1, "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/matching1.csv")

# function to see which countries do not match well with gadm shapefiles
nomatch <- function(matchList, countries){
percentNotMatch_shpfile <- c()
for (i in 1:length(matchList)){
  percentNotMatch_shpfile[i] <- nrow(matchList[[i]][merge_status == "in WHO but not shpfile"])*100/nrow(matchList[[i]][merge_status != "in shpfile but not WHO"])
}
percentNotMatch <- data.table(countries, percentNotMatch_shpfile)
return(percentNotMatch)
}
notMatch_gadm <- nomatch(gadm_afr_match, africa_countries)
notMatch_gaul <- nomatch(gaul_afr_match, africa_countries)
...
final_df_gadm <- dataTable(gadm_afr_match)
final_df_gaul <- dataTable(gaul_afr_match)

#write.csv(final_df, "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/who_gadm_matching_Africa.csv")
#write.csv(final_df2, "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/who_gaul_matching_Africa.csv")

#country with at least 30% match with gaul and gadm shapefile
notMatch <- cbind(notMatch_gadm, notMatch_gaul$percentNotMatch_shpfile)
setnames(notMatch, c(2,3), c("gadm", "gaul"))
#sort out countries match with gaul shapefile better than with gadm shapefile and vice versa
notMatch[gadm == gaul & gadm < 30, shapefile := "gaul/gadm"]
notMatch[gaul < gadm & gaul < 30, shapefile := "gaul"]
notMatch[gadm < gaul & gadm < 30, shapefile := "gadm"]
#doesnot match by both gadm and gaul shapefile will be NA in the table
table(is.na(notMatch$shapefile))


#list of countries match gadm shapefile
gadm_afr_countries <- notMatch[shapefile == "gadm", 1][[1]]

#list of countries match gaul shapefile
gaul_afr_countries <- notMatch[shapefile == "gaul" | shapefile == "gaul/gadm", 1][[1]]

#rerun matching using only gadm countries
gadm_afr_match <- matching(gadm_afr_countries, df_gadm, "osa")
#rerun matching using only gaul countries
gaul_afr_match <- matching(gaul_afr_countries, df_gl, "osa")

final_df_gaul <- dataTable(gaul_afr_match)
final_df_gadm <- dataTable(gadm_afr_match)
#write.csv(final_df_gadm, "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/who_gadm_matching_Africa.csv")
#write.csv(final_df_gaul, "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/who_gaul_matching_Africa.csv")

# country with 100% unmatch to gaul and gadm shapefile
nomatch_gaulcountries <- notMatch[gaul == 100 & gadm == 100, 1][[1]]
#rerun on those 100% unmatch countries
nomatch_gaulmatch <- matching(nomatch_countries, df_gl, "osa")

df_nomatch <- dataTable(nomatch_match)
write.csv(df_nomatch, "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/who_gaul_100_nomatch_Africa.csv")
#Rwanda is removed 
gaul_adm1_match <- matching_admin1(nomatch_gaulcountries[-12], df_gl1, "osa")
nomatch_adm1 <- nomatch(gaul_adm1_match, nomatch_gaulcountries[-12])
df_gl_adm1 <- dataTable(gaul_adm1_match)



# run individual country
# run remaining unmatched gaul countries in previous run
countries_adm2 <- c("Cape Verde", "Gabon", "Niger", "Rwanda", "Sierra Leone","Tanzania"
                    , "South Africa")
adm2_match <- matching_admin2(countries_adm2,df_gl,"osa") 
nomatch(adm2_match, countries_adm2)
df_adm2_match <- dataTable(adm2_match)
write.csv(df_adm2_match , "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/remaining_adm2_gl.csv")

# run remaining unmatched gadm countries in previous run
countries_gadm_adm2 <- c("Botswana", "Sao Tome and Principe")
adm2_match <- matching_admin2(countries_gadm_adm2, df_gadm, "osa")
df_adm2_match <- dataTable(adm2_match)
write.csv(df_adm2_match , "C:/Users/nqviet94/Documents/IHME/Work/Vaccine (local)/WHO_Subnational_Immunization_Coverage_data/2016/remaining_adm2_gadm.csv")
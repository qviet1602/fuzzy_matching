...
setnames(df_who, c("Iso Code", "Country Name", "Year", "Admin1", "Admin2", "Coverage", "Vaccine Type"),
         c("iso3", "country", "year", "ADM1_NAME", "ADM2_NAME", "who_coverage", "vaccine"))
# choose vaccine
...
#count country
africa_countries <- unique(df_who$country, by= "country")
...
# start the for loop matching for all african countries using gadm shapefile
  for (i in 1 : length(africa_countries)){
    df_combined[[i]]<- merge(df_who[country == africa_countries[i]], df_gadm[ADM0_NAME == africa_countries[i]], by = c("ADM1_NAME", "ADM2_NAME"), all.x = T, all.y = T)
    #regular match status
    df_combined[[i]][!is.na(who_coverage) & !is.na(GAUL_CODE), merge_status := "success"]
    df_combined[[i]][!is.na(who_coverage) & is.na(GAUL_CODE), merge_status := "in WHO but not gadm"]
    df_combined[[i]][is.na(who_coverage) & !is.na(GAUL_CODE), merge_status := "in gadm but not WHO"]
    #subset each status to different list for fuzzy match
    df_who_only[[i]] <- subset(df_combined[[i]], merge_status == "in WHO but not gadm")
    df_gadm_only[[i]] <- subset(df_combined[[i]], merge_status == "in gadm but not WHO")
    df_success[[i]] <- subset(df_combined[[i]], merge_status == "success")
    
    #fuzzy match
    df_fuzzy[[i]] <- stringdist_join(subset(df_who_only[[i]], select = c(1:6)), subset(df_gadm_only[[i]], select = c(1,2,8,9,10,11)), by = "ADM2_NAME", method = "osa")
    ...
    setnames(df_fuzzy_fix[[i]], c("ADM2_NAME.x", "ADM2_NAME.y"), c("ADM2_NAME.who", "ADM2_NAME.gadm"))
    # fuzzy match with only one gadm will have NA on merge status
    df_fuzzy_exact[[i]] <- df_fuzzy[[i]][is.na(merge_status)]
    setnames(df_fuzzy_exact[[i]], c("ADM2_NAME.x", "ADM2_NAME.y"), c("ADM2_NAME.who", "ADM2_NAME.gadm"))
    ...
    #sort out adm which never get match (regular and fuzzy match)
    df_combined2[[i]] <- unique(df_combined[[i]], by= c("who_coverage", "ADM2_NAME.who"))
  
    #final data set with matching
    df_combined_final[[i]] <- rbind(df_combined[[i]][merge_status != "in WHO but not gadm"], df_combined2[[i]][merge_status == "in WHO but not gadm",])
  }

#see which countries do not match well with gadm shapefiles
percent_not_match_gadm <- c()
for (i in 1:46){
  percent_not_match_gadm[i] <- nrow(df_combined_final[[i]][merge_status == "in WHO but not gadm"])*100/nrow(df_combined_final[[i]])
}

percent_not_match <- data.table(africa_countries, africa_iso3, percent_not_match_gadm)

#convert list to data table
final_df <- c()
for (i in 1:(length(df_combined_final))){
final_df <- rbind(final_df, df_combined_final[[i]])
}
setnames(final_df, c("ADM1_NAME.x", "ADM1_NAME.y"), c("ADM1_NAME.who", "ADM1_NAME.gadm"))
final_df <- final_df[,c(1:5,7,10,11,12,14,13,15,6)]
...
#using gaul shape files matching for flagged countries
...
for (i in 1 : length(gaul_afr_countries)){
  df_combined[[i]]<- merge(df_who[country == gaul_afr_countries[i]], df_gl[ADM0_NAME == gaul_afr_countries[i]], by = c("ADM1_NAME", "ADM2_NAME"), all.x = T, all.y = T)
  #regular match status
  df_combined[[i]][!is.na(who_coverage) & !is.na(GAUL_CODE), merge_status := "success"]
  df_combined[[i]][!is.na(who_coverage) & is.na(GAUL_CODE), merge_status := "in WHO but not gaul"]
  df_combined[[i]][is.na(who_coverage) & !is.na(GAUL_CODE), merge_status := "in gaul but not WHO"]
  #subset each status to different list for fuzzy match
  df_who_only[[i]] <- subset(df_combined[[i]], merge_status == "in WHO but not gaul")
  df_gl_only[[i]] <- subset(df_combined[[i]], merge_status == "in gaul but not WHO")
  df_success[[i]] <- subset(df_combined[[i]], merge_status == "success")
  
  #fuzzy match
  df_fuzzy[[i]] <- stringdist_join(subset(df_who_only[[i]], select = c(1:6)), subset(df_gl_only[[i]], select = c(1,2,8,9,10,11)), by = "ADM2_NAME", method = "osa")
  df_fuzzy[[i]]$merge_status <- NA
  #identify fuzzy match with more than 1 match
  ...
  df_fuzzy_fix[[i]] <- df_fuzzy[[i]][merge_status == "fuzzy need fix"]
  setnames(df_fuzzy_fix[[i]], c("ADM2_NAME.x", "ADM2_NAME.y"), c("ADM2_NAME.who", "ADM2_NAME.gl"))
  # fuzzy match with only one gadm will have NA on merge status
  df_fuzzy_exact[[i]] <- df_fuzzy[[i]][is.na(merge_status)]
  setnames(df_fuzzy_exact[[i]], c("ADM2_NAME.x", "ADM2_NAME.y"), c("ADM2_NAME.who", "ADM2_NAME.gl"))
  ...
  #final data set with matching
  df_combined_final[[i]] <- rbind(df_combined[[i]][merge_status != "in WHO but not gaul"], df_combined2[[i]][merge_status == "in WHO but not gaul",])
}
# #see which countries do not match well with gaul shapefiles
percent_not_match_gaul <- c()
for (i in 1:46){
  percent_not_match_gaul[i] <- nrow(df_combined_final[[i]][merge_status == "in WHO but not gaul"])*100/nrow(df_combined_final[[i]])
}

percent_not_match <- cbind(percent_not_match, percent_not_match_gaul)


#sort out countries match with gaul shapefile better than with gadm shapefile and vice versa
percent_not_match[percent_not_match_gadm == percent_not_match_gadm & percent_not_match_gadm < 30, shapefile := "gaul/gadm"]
percent_not_match[percent_not_match_gaul < percent_not_match_gadm & percent_not_match_gaul < 30, shapefile := "gaul"]
percent_not_match[percent_not_match_gadm < percent_not_match_gaul & percent_not_match_gadm < 30, shapefile := "gadm"]
#doesnot match by both gadm and gaul shapefile will be NA in the table
table(is.na(percent_not_match$shapefile))

#convert list to data table
final_df2 <- c()
for (i in 1:(length(df_combined_final))){
  final_df2 <- rbind(final_df2, df_combined_final[[i]])
}
setnames(final_df2, c("ADM1_NAME.x", "ADM1_NAME.y"), c("ADM1_NAME.who", "ADM1_NAME.gl"))
final_df2 <- final_df2[,c(1:5,7,10,11,12,14,13,15,6)]
...
gadm_afr_countries <- percent_not_match[shapefile == "gadm" | shapefile == "gaul/gadm", 1]
gadm_afr_countries <- gadm_afr_countries[[1]]
gaul_afr_countries <- percent_not_match[shapefile == "gaul" | shapefile == "gaul/gadm", 1]
gaul_afr_countries <- gaul_afr_countries[[1]]


# percent_not_match_osa <- percent_not_match

# percent_not_match_soundex <- percent_not_match

# sum(percent_not_match_osa[percent_not_match_gadm < 30, percent_not_match_gadm]) /length(percent_not_match_osa[percent_not_match_gadm < 30, percent_not_match_gadm])
# sum(percent_not_match_osa[percent_not_match_gaul <30, percent_not_match_gaul]) / length(percent_not_match_osa[percent_not_match_gaul <30, percent_not_match_gaul])
# 
# 
# sum(percent_not_match_soundex[percent_not_match_gadm < 30, percent_not_match_gadm]) /length(percent_not_match_soundex[percent_not_match_gadm < 30, percent_not_match_gadm])
# sum(percent_not_match_soundex[percent_not_match_gaul <30, percent_not_match_gaul]) / length(percent_not_match_soundex[percent_not_match_gaul <30, percent_not_match_gaul])
# 
# 
# percent_not_match_osa[,shapefile_soundex:= percent_not_match_soundex$shapefile]
